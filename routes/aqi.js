...
router.get('/cities', async (req, res, next) => {
  try {
    ...
    try {
      ...
      // IQAir data inconsistency: state appears in /states list but /cities rejects it.
      // Return an empty list rather than crashing with a 500.
      if (iqairError.message && (
        iqairError.message.includes('state_not_found') ||
        iqairError.message.includes('400')
      )) {
        console.warn(`⚠️ IQAir state_not_found for state="${state}" country="${country}" — returning empty cities list`);
        return res.json({ success: true, data: { state, country, count: 0, cities: [] } });
      }
      throw iqairError; // re-throw anything unexpected
    }
  } catch (error) { next(error); }
});
